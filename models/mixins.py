...
class GeoTiffPredictionMixin(pl.LightningModule, metaclass=ABCMeta):
    def pred_dataloader(self, geotiff_path: str):
        ds_pred = GeoTiffReader(
            geotiff_path,
            transform=t.geotiff_transforms,
            crop_size=self.hparams.crop_size,
            padding=self.hparams.crop_pad,
        )
        return DataLoader(
            ds_pred,
            shuffle=False,
    ...
    def predict_geotiff(self, geotiff_path: str, out_path: str):
        ...
        writer = CropDatasetWriter.from_reader(
            out_path, pred_dataloader.dataset, count=1, dtype="uint8", nodata=0
        )
