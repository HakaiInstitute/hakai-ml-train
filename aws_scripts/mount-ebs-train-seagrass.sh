...
VOLUME_ID=$(aws ec2 describe-volumes --region $AWS_REGION --filter "Name=tag:Name,Values=DL-seagrass-checkpoints" --query "Volumes[].VolumeId" --output text)
VOLUME_AZ=$(aws ec2 describe-volumes --region $AWS_REGION --filter "Name=tag:Name,Values=DL-seagrass-checkpoints" --query "Volumes[].AvailabilityZone" --output text)
...
if [ $VOLUME_ID ]; then
		...
		if [ $VOLUME_AZ != $INSTANCE_AZ ]; then
				SNAPSHOT_ID=$(aws ec2 create-snapshot \
						...
						--tag-specifications 'ResourceType=snapshot,Tags=[{Key=Name,Value=DL-seagrass-checkpoints-snapshot}]' \
		...
		sudo -H -u ubuntu bash -c "source /home/ubuntu/anaconda3/bin/activate python3; bash ./unet/seagrass/build_and_run.sh"
fi
