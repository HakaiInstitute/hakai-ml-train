...
def eval_model(model, data_loaders, num_classes, criterion):
    model.eval()
    ...
    for phase in ['train', 'eval']:
        ...
        for x, y in tqdm(iter(data_loaders[phase]), desc=phase):
            y = y.to(device)
            x = x.to(device)
...
if __name__ == '__main__':
    ...
    if DOCKERIZED:
        ...
        output_dir = Path('kelp/train_output/model_weights')
        stderr_file_path = Path('kelp/train_output/logs/failure')
    ...
    if sys.argv[1] == "train":
        ...
        # Trained model
        model_weights_path = output_dir.joinpath('200407/deeplabv3_best_val_loss.pt')
        ...
        raise RuntimeError("Must specify train|eval as arg")
