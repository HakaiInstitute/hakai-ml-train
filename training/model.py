...
class UNetPlusPlus(pl.LightningModule):
        ...
        if has_wandb_logger:
            for phase in ["train", "val"]:
                self.logger.experiment.define_metric(
                    f"{phase}/loss", goal="minimize", summary="min"
                )
                self.logger.experiment.define_metric(
                    f"{phase}/miou", goal="maximize", summary="max"
                )
                self.logger.experiment.define_metric(
                    f"{phase}/accuracy", goal="maximize", summary="max"
                )
                ...
                for c in range(self.n):
                    ...
                    self.logger.experiment.define_metric(
                        f"{phase}/{clsx}_iou", goal="maximize", summary="max"
                    )
                    self.logger.experiment.define_metric(
                        f"{phase}/{clsx}_recall", goal="maximize", summary="max"
                    )
                    self.logger.experiment.define_metric(
                        f"{phase}/{clsx}_precision", goal="maximize", summary="max"
                    )
                    self.logger.experiment.define_metric(
                        f"{phase}/{clsx}_f1", goal="maximize", summary="max"
                    )
    ...
    def configure_optimizers(self):
        ...
        cosine_sch = CosineAnnealingLR(optimizer, T_max=(steps - warmup_steps))
