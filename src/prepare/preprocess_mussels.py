...
def remap_label(labels, device=None):
    ...
    # Index 0->0, 1->1, -100 for everything else
    lookup = torch.tensor([0, 1], dtype=labels.dtype, device=device)
...
class ImageMosaicDataset(RasterDataset):
    is_image = True
...
def load_dataset(data_dir, img_name):
    images = ImageMosaicDataset(paths=[str(data_dir)], img_name=img_name)
    labels = LabelsDataset(paths=[str(data_dir)], img_name=img_name)
