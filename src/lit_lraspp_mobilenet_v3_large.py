...
class LRASPPMobileNetV3Large(pl.LightningModule):
    def __init__(self, num_classes: int = 2, ignore_index: Optional[int] = None,
                 lr: float = 0.35, weight_decay: float = 3e-6):
        super().__init__()
        ...
        # Loss function
        self.focal_tversky_loss = FocalTverskyMetric(self.num_classes,
    ...
    def predict_step(self, batch: Any) -> Any:
        ...
        pred = torch.vstack(
            [self.forward(batch).unsqueeze(0) for _ in range(mc_iteration)]).mean(dim=0)
        return pred
