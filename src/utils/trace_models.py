...
if __name__ == "__main__":
    ...
    model = LRASPPMobileNetV3Large(
        {"weight_decay": 0.001, "lr": 0.001, "num_classes": 2,}
    )
    model.load_state_dict(
        torch.load(
    ...
    model = LRASPPMobileNetV3Large(
        {"weight_decay": 0.001, "lr": 0.001, "num_classes": 3,}
    )
    model.load_state_dict(
        torch.load(
    ...
    model.load_state_dict(
        torch.load(
            "scripts/mussels/train_output/checkpoints/lr_aspp_mussels/checkpoints/"
            "best-val_miou=0.7770-epoch=96-step=19884.pt"
        )
    )
    model = model.eval()
    model = model.to(DEVICE)
    traced_model = torch.jit.trace(model.model, x, strict=False)
    traced_model.save("torchscript_files/LRASPP_MobileNetV3_mussel_presence_jit.pt")
    # torch.onnx.export(model, x, "torchscript_files/LRASPP_MobileNetV3_kelp_species.onnx",
    #                   opset_version=11, verbose=True)

    # Deeplab Mussel Model
    model = DeepLabv3ResNet101(
        {
            "aux_loss_factor": 0.3,
            "weight_decay": 0.001,
    ...
    model.load_state_dict(
        torch.load(
            "scripts/mussels/train_output/checkpoints/deeplabv3_mussels/checkpoints/"
            "best-val_miou=0.8303-epoch=97-step=20089.pt"
        )
    )
    model = model.eval()
    model = model.to(DEVICE)
    traced_model = torch.jit.trace(model.model, x, strict=False)
    traced_model.save("torchscript_files/DeepLabV3_ResNet101_mussel_presence_jit.pt")
    # torch.onnx.export(model, x, "torchscript_files/DeepLabV3_ResNet101_kelp_presence.onnx",
    #                   opset_version=11, verbose=True)
