...
MODEL_WEIGHTS = (
    "./weights/UNetPlusPlus_EfficientNetB4_kelp_presence_aco_jit_miou=0.8785.pt"
)
...
class HanningWindowSegmentation(object):
    ...
    def _pad_tile(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return torch.nn.functional.pad(
            x, (0, self.ws - tw, 0, self.ws - th), mode="constant", value=0
        )
    ...
    def __call__(self, img_path: str, output_path: str):
        ...
        with rasterio.open(img_path) as src, rasterio.open(output_path, "r+") as dest:
            ...
            for window in tqdm(windows):
                x = torch.tensor(
                    src.read(window=window), device=self.device, dtype=torch.uint8
                )
                x = self._pad_tile(x)
...
def find_kelp(img_path, output_path):
    ...
    segmentor = HanningWindowSegmentation(
        model, BartlettHanningKernel, NUM_CLASSES, CLIP_SIZE, DEVICE
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("img_path", type=Path, help="Path to the image to be segmented")
    parser.add_argument(
        "output_path", type=Path, help="Path to the output segmentation"
    )
