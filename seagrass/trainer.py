...
from seagrass.model import SeagrassModel
...
def train(train_data_dir, val_data_dir, checkpoint_dir,
          num_classes=2, batch_size=4, lr=0.001, weight_decay=1e-4, epochs=100, aux_loss_factor=0.3,
    ...
    if checkpoint:
        ...
        model = SeagrassModel.load_from_checkpoint(checkpoint, hparams=hparams)
        trainer = pl.Trainer(resume_from_checkpoint=checkpoint, **trainer_kwargs)
    else:
        model = SeagrassModel(hparams)
